document.getElementById('filter_place_button').addEventListener('click', function() {
   // Получаем значения из полей формы
    let minRating = document.getElementById('min_rating').value;
    let maxRating = document.getElementById('max_rating').value;
    let userId = document.getElementById('users').value;
    let filterTagsArray = document.getElementById('filter_tags').value.split(' '); // Преобразуем строку в массив

    // Создаем начало URL запроса
    var jsonUrl = 'http://localhost:8080/api/v1/places';

    // Переменная для хранения символа перед первым параметром
    var separator = '?';

    // Добавляем параметры запроса в зависимости от заполненных полей
    if (minRating && !maxRating) {
        jsonUrl += `${separator}ratingStart=${minRating}`;
        separator = '&'; // изменяем разделитель на &
    } else if (!minRating && maxRating) {
        jsonUrl += `${separator}ratingEnd=${maxRating}`;
        separator = '&'; // изменяем разделитель на &
    } else if (minRating && maxRating) {
        jsonUrl += `${separator}ratingStart=${minRating}&ratingEnd=${maxRating}`;
        separator = '&'; // изменяем разделитель на &
    }

    // Если указан id пользователя, добавляем его в запрос
    if (userId) {
        jsonUrl += `${separator}user=${userId}`;
        separator = '&'; // изменяем разделитель на &
    }

    // Если указаны тэги, добавляем их в запрос
    if (filterTagsArray && filterTagsArray.length > 0) {
        // Преобразуем массив тэгов обратно в строку, разделяя их запятой
        let filterTagsString = filterTagsArray.join('&tags=');
        jsonUrl += `${separator}tags=${filterTagsString}`;
        separator = '&'; // изменяем разделитель на &
    }
    console.log(jsonUrl);
    sessionStorage.setItem('filterParams', jsonUrl);
...
document.getElementById('reset_filter').addEventListener('click', function() {
    ...
    document.getElementById('filter_tags').value = '';
});
